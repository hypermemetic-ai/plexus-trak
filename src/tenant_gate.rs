...
use plexus_auth_core::{ClaimTenantResolver, Tenant, TenantResolver};
...
impl TenantGate {
    ...
    /// 2. Otherwise, the `AuthContext` is mirrored into a
    ///    `plexus_auth_core::AuthContext` (the resolver's expected shape)
    ///    and passed to [`ClaimTenantResolver::new`] (claim key
    ///    `"tenant_id"`, `single_user_fallback = true`).
    /// 3. On resolver success → `tenant = Some(...)`.
    /// 4. On resolver error (anonymous, missing claim, malformed) →
    ///    `tenant = None`.
    ///
    /// **Cross-crate `AuthContext` note:** plexus-trak currently consumes
    /// `plexus-core 0.5.2` from crates.io, which predates the AUTHZ-CORE-CRATE-1
    /// migration that moved `AuthContext` into `plexus-auth-core`. The two
    /// `AuthContext` types are structurally identical (same `pub` fields)
    /// but distinct types from the type-system's POV. We mirror the fields
    /// at the gate boundary so the `ClaimTenantResolver` primitive can be
    /// exercised end-to-end without forking the field shape.
        ...
        let tenant = match auth {
            ...
            Some(ctx) => {
                ...
                if !ctx.is_authenticated() {
                    ...
                    resolver.resolve(&mirror).await.ok()
                }
            }
        };
        Self { store, tenant }
